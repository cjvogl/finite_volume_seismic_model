...
c     #########################################
c     THIS SHOULD EVENTUALLY BE SET DYNAMICALLY
      parameter (maux1d = 18)
c     #########################################
      real(kind=8), intent(in) :: xlow, ylow, zlow, t
      real(kind=8) :: xval, yval, zval


      external rpn3,rpt3,rptt3,setaux1d
...
c
     ...
     & aux(maux, 1-mbc:maxmx+mbc, 1-mbc:maxmy+mbc, 1-mbc:maxmz+mbc)
      dimension aux1(maux1d, 1-mbc:maxm+mbc, 3)
      dimension aux2(maux1d, 1-mbc:maxm+mbc, 3)
      dimension aux3(maux1d, 1-mbc:maxm+mbc, 3)
...
c
      do 50 k = 0,mz+1
         zval = zlow + (mbc+k-0.5d0)*dz
         do 50 j = 0,my+1
           yval = ylow + (mbc+j-0.5d0)*dy
c
...
c           # this slice:
c
            call flux3(1,maxm,meqn,maux1d,mbc,mx,
...
c           # slice:
c
            call flux3(2,maxm,meqn,maux1d,mbc,my,
...
c           # slice:
c
            call flux3(3,maxm,meqn,maux1d,mbc,mz,
