...
!  in (qinit.f), b.c.'s in (bc3amr.f).
!
!  Specify rectangular domain from
!           (xlower,ylower,zlower) to (xupper,yupper,zupper).
!
!
! =========================================================================
!
...
program amr3
    ...
    use amr_module, only: xlower, ylower, zlower
    use amr_module, only: hxposs, hyposs, hzposs
    use amr_module, only: intratx, intraty, intratz, kratio
    ...
    common /comfine/ dxmin,dymin, dzmin
    ...
    zperdom =  (mthbc(5) == 2 .and. mthbc(6) == 2)
    !! spheredom =  (mthbc(3) == 5 .and. mthbc(4) == 5) NOT IMPLEMENTED
    ...
    if ((mthbc(5).eq.2 .and. mthbc(6).ne.2) .or. &
        ...
        print *, '*** ERROR ***  periodic boundary conditions: '
        print *, '  mthbc(5) and mthbc(6) must BOTH be set to 2'
        stop
    endif
...
!$   maxthreads = omp_get_max_threads()
    ...
    write(outunit,format_string) xupper,yupper,zupper,      &
